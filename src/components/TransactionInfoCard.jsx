...
const TransactionInfoCard=({icon,title,date,amount,type,hideDeleteBtn,onDelete})=>{
    ...
    return(
        <div className="group relative flex items-center gap-4 mt-2 p-3 rounded-lg hover:bg-gray-100/60">
            <div className="w-12 h-12 flex items-center justify-center text-xl text-gray-800 bg-gray-100 rounded-full">
                {icon?(<img src={icon} alt={title} className="w-6 h-6"></img>):(<UtensilsCrossed className="text-purple-500"></UtensilsCrossed>)}
            </div>
            <div className="flex-1 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    {!hideDeleteBtn && (<button onClick={()=>onDelete()} className="text-gray-400 hover:text-red-800 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
 ...
 <h6 className="text-xs font-medium">
                  ...
                  {type=="income"?(<TrendingUp size={15}></TrendingUp>):(<TrendingDown size={15}></TrendingDown>)}
                    </div>
                </div>
            </div>
        </div>
